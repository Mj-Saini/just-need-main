...
function AddSubCategoryPopUp({
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      if (isEditMode && initialData?.id) {
        const updatedData = { categoryName: subCategoryName };
        const success = await updateSubcategoryName(initialData.id, updatedData);
        if (success) {
          if (onSuccess) onSuccess();
          handleClose();
        } else {
      ...
      console.error(`Error ${isEditMode ? "updating" : "adding"} subcategory:`, error.message);
      toast.error(`Failed to ${isEditMode ? "update" : "add"} subcategory: ${error.message}`);
    }
  };
